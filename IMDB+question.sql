...
-- Number of movies released on each year.
	SELECT 
		year, 
...
-- Number of movies released on each month.
	...
	WITH rank_on_avg_rating AS (
		...
		FROM 
			ratings AS r 
				INNER JOIN movie AS m 
					ON m.id = r.movie_id
	)	SELECT * FROM rank_on_avg_rating LIMIT 10;
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
	WITH highest_hit_production_company AS (
		SELECT 
			production_company,
	...
	SELECT * FROM highest_hit_production_company WHERE prod_company_rank = 1;
...
-- Type your code below:
	...
	FROM 
		...
		INNER JOIN movie AS m
			ON m.id=g.movie_id
	WHERE country LIKE '%USA%' AND year = 2017 AND r.total_votes > 1000 AND MONTH(date_published) = 3
	GROUP BY g.genre
	ORDER BY movie_count DESC;
...
-- Type your code below:
		...
		SELECT 
			country,
			SUM(total_votes) as sum_of_total_votes
		FROM 
			movie as m 
				INNER JOIN ratings as r 
					ON r.movie_id=m.id
...
-- Type your code below:
	...
	SELECT * FROM top_production_companies WHERE prod_comp_rank <= 3;
...
-- Top 3 Genres based on most number of movies
	...
	SELECT * FROM top5_movies WHERE movie_rank <= 5;
...
-- Type you code below:
	...
	WITH movies_released_date_summary AS
	(
		SELECT 
			dm.name_id,
			NAME,
	...
	(
		...
		FROM movies_released_date_summary
	)
	SELECT 
